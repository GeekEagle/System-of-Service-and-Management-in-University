...
public class Jstudenthonorupdata extends JFrame{
    public Jstudenthonorupdata(String ID) {
        this.setSize(1000,600);
        this.setTitle("学生荣誉信息更新");
        Container pane=this.getContentPane();
        pane.setSize(1800,1500);
        //pane.setLayout(new FlowLayout(FlowLayout.LEFT));
        String[] listname= {"操作号","学号","姓名","专业",
                "获得荣誉时间","荣誉名","荣誉等级"};
        ...
        try {
            ...
            String SQL1 ="select count(*) from 获奖记录表_view where StuID="+ID;
            ...
            String SQL = "select * from 获奖记录表_view where StuID="+ID;
            stmt = con.createStatement();
            int i=0;
            rs =stmt.executeQuery(SQL);
            while(rs.next()) {
                count[i]=new String[] {rs.getString("Awanum"),rs.getString("StuID"),
                        rs.getString("Stuname"),rs.getString("Stupro"),
                        rs.getString("Awardate"),rs.getString("Awarea"),
                        rs.getString("Awagra")};
                i++;
            }
        }
        ...
        */
        ...
        JLabel txt5 = new JLabel("荣誉名称：");txt5.setFont(new Font("宋体",Font.PLAIN,20));
        JTextField txthname = new JTextField(20);
        pane.add(txt5);
        pane.add(txthname);
        txt5.setBounds(500,700,150,40);txthname.setBounds(700,700,150,40);

        JLabel txt6 = new JLabel("荣誉等级：");
        JTextField txthlevel = new JTextField(20);
        pane.add(txt6);
        pane.add(txthlevel);
        txt6.setBounds(20,780,150,40);txthlevel.setBounds(200,780,150,40);

        JLabel txt7 = new JLabel("需要修改的荣誉编号：");
        JTextField txtdonum = new JTextField(20);
        pane.add(txt7);
        pane.add(txtdonum);
        txt7.setBounds(500,780,150,40);txtdonum.setBounds(700,780,150,40);

        JButton btnOK=new JButton("确定插入学生荣誉信息");
        pane.add(btnOK);
        ...
        btnOK.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(txtName.getText().length()==0 || txtMaj.getSelectedItem().toString().length()==0
                        ||txttime.getText().length()==0||txthname.getText().length()==0) {
                    JOptionPane.showMessageDialog(frame, "姓名，专业，获得时间，荣誉名称必填！");
                    return;
                }
                // Create a variable for the connection string.
                String connectionUrl = "jdbc:sqlserver://localhost:1433;DatabaseName=dbkeshe";
                ...
                try {
                    ...
                    con = DriverManager.getConnection(connectionUrl, "sa", "159357");
                    // Create and execute an SQL statement that returns some data.
                    c=con.prepareCall("{call 获奖插入(?,?,?,?,?,?)}");
                    c.setString(1,ID);
                    c.setString(2,txtName.getText());
                    c.setString(3,txtMaj.getSelectedItem().toString());
                    c.setString(4,txttime.getText());
                    c.setString(5,txthname.getText());
                    c.setString(6,txthlevel.getText());
                    c.execute();
                    c.close();
                    frame.setVisible(false);
                    Jstudenthonorupdata f = new Jstudenthonorupdata(ID);
                    f.setVisible(true);
                }
                // Iterate through the data in the result set and display it.
                // Handle any errors that may have occurred.
        ...
        JButton btnup=new JButton("确定更新学生荣誉信息");
        pane.add(btnup);
        ...
        btnup.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(txtdonum.getText().length()==0||txtName.getText().length()==0 || txtMaj.getSelectedItem().toString().length()==0
                        ||txttime.getText().length()==0||txthname.getText().length()==0) {
                    JOptionPane.showMessageDialog(frame, "操作编号，姓名，专业，获得时间，荣誉名称必填！");
                    return;
                }
                // Create a variable for the connection string.
                String connectionUrl = "jdbc:sqlserver://localhost:1433;DatabaseName=dbkeshe";
                ...
                try {
                    ...
                    c.setString(6,txthname.getText());
                    c.setString(7,txthlevel.getText());
                    c.execute();
                    c.close();
                    frame.setVisible(false);
                    Jstudenthonorupdata f = new Jstudenthonorupdata(ID);
        ...
        JButton btndel=new JButton("确定删除学生荣誉信息");
        pane.add(btndel);
